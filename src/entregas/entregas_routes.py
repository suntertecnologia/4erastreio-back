...
def scrap_and_save(scrap_request: entregas_models.EntregaScrapRequest, user_id: int):
    ...
    try:
        ...
        if scraped_data["status"] == "sucesso":
            logger.info(
                f"Scraping successful for {scrap_request.transportadora} - NF {scrap_request.numero_nf}. Data: {scraped_data['dados']}"
            )
            ...
            logger.info(
                f"Successfully saved scraped data for {scrap_request.transportadora} - NF {scrap_request.numero_nf}"
            )
        else:
            logger.error(
                f"Scraping failed for {scrap_request.transportadora} - NF {scrap_request.numero_nf}: {scraped_data['erro']}"
            )
