...
def normalize_braspress(data):
    ...
    if "ENTREGA REALIZADA" in last_event['status']:
        ...
        previsao_entrega = datetime.strptime(previsao_entrega_str, '%d/%m/%Y').strftime('%Y-%m-%d')
...
def normalize_accert(data):
    ...
    # Extract history
    history_blocks = re.findall(r"(\d{2}/\d{2} \d{2}:\d{2})\n\n(.*?)\n\n(.*?)(?=\n\n\d{2}/\d{2} \d{2}:\d{2}|$)", details_str, re.DOTALL)
    
    normalized_history = []
    for block in history_blocks:
        timestamp_str, status, details = block
        timestamp = datetime.strptime(f"{timestamp_str}/{datetime.now().year}", '%d/%m %H:%M/%Y').isoformat()
    ...
    if normalized_history:
        post_date = datetime.fromisoformat(normalized_history[0]['timestamp']).strftime('%Y-%m-%d')
...
def normalize_viaverde(data):
    ...
    if data.get('data_entrega'):
        previsao_entrega = datetime.strptime(data['data_entrega'], '%d/%m/%Y').strftime('%Y-%m-%d')
...
def normalize_jamef(data):
    ...
    history_blocks = re.findall(r"Data: (.*?)\n\nStatus: (.*?)\n\nEstado origem: (.*?)\n\nMunicípio origem: (.*?)\n\nEstado destino: (.*?)\n\nMunicípio destino: (.*?)(?=\n\nData:|$)", details_str, re.DOTALL)
    ...
    for block in history_blocks:
        ...
        local = {"cidade": cidade_destino.strip(), "estado": None} # State is not available
    ...
    post_date = None
    if normalized_history:
        post_date = datetime.fromisoformat(normalized_history[-1]['timestamp']).strftime('%Y-%m-%d')

    previsao_entrega = None
    if normalized_history and "ENTREGA REALIZADA" in normalized_history[0]['status']:
        previsao_entrega = datetime.fromisoformat(normalized_history[0]['timestamp']).strftime('%Y-%m-%d')
